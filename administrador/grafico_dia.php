...
<body>
    ...
    <div>
        <ul class="migas-de-pan">
            ...
            <li class="migas-de-pan-item"><a href="bienvenido_administrador.php" class="migas-de-pan-link" style="font-weight: 600;">Gráfico Específico por día</a></li>
        </ul>
    </div>
    ...
    <div style="padding-left:30px; padding-right:30px">
        ...
        <?php
        $fecha = $_POST['fecha'];
        $query = "SELECT count(id_solicitud) AS cantidad_solicitudes_fecha FROM solicitud WHERE fecha = '$fecha'";
        $result = mysqli_query($conexion, $query);
        $row = mysqli_fetch_assoc($result);
        $cantidad_solicitudes_fecha = $row['cantidad_solicitudes_fecha'];

        $query = "SELECT count(id_solicitud) AS solicitudes_felicitacion_fecha FROM solicitud WHERE fecha = '$fecha' AND solicitud = 'felicitacion'";
        $result = mysqli_query($conexion, $query);
        $row = mysqli_fetch_assoc($result);
        $solicitudes_felicitacion_fecha = $row['solicitudes_felicitacion_fecha'];

        $query = "SELECT count(id_solicitud) AS solicitudes_reclamos_fecha FROM solicitud WHERE fecha = '$fecha' AND solicitud = 'reclamos'";
        $result = mysqli_query($conexion, $query);
        $row = mysqli_fetch_assoc($result);
        $solicitudes_reclamos_fecha = $row['solicitudes_reclamos_fecha'];

        $query = "SELECT count(id_solicitud) AS solicitudes_sugerencia_fecha FROM solicitud WHERE fecha = '$fecha' AND solicitud ='sugerencia'";
        $result = mysqli_query($conexion, $query);
        $row = mysqli_fetch_assoc($result);
        $solicitudes_sugerencia_fecha = $row['solicitudes_sugerencia_fecha'];

        $query = "SELECT count(id_solicitud) AS solicitudes_en_proceso_fecha FROM solicitud WHERE fecha = '$fecha' AND estado = 'en proceso'";
        $result = mysqli_query($conexion, $query);
        $row = mysqli_fetch_assoc($result);
        $solicitudes_en_proceso_fecha = $row['solicitudes_en_proceso_fecha'];

        $query = "SELECT count(id_solicitud) AS solicitudes_terminada_fecha FROM solicitud WHERE fecha = '$fecha' AND estado = 'terminado'";
        $result = mysqli_query($conexion, $query);
        $row = mysqli_fetch_assoc($result);
        $solicitudes_terminada_fecha = $row['solicitudes_terminada_fecha'];

        if ($cantidad_solicitudes_fecha == 0 && $solicitudes_felicitacion_fecha == 0 && $solicitudes_reclamos_fecha == 0 && $solicitudes_sugerencia_fecha == 0 && $solicitudes_en_proceso_fecha == 0 && $solicitudes_terminada_fecha == 0) {
            echo "<h1>Lamentablemente en este dia no se ingresaron solicitudes...</h1>";
        }
        ?>
        <div id="apex2">
        ...
        <script>
            var cantidad_solicitudes_fecha = <?php echo json_encode($cantidad_solicitudes_fecha); ?>;
            var solicitudes_felicitacion_fecha = <?php echo json_encode($solicitudes_felicitacion_fecha); ?>;
            var solicitudes_reclamos_fecha = <?php echo json_encode($solicitudes_reclamos_fecha); ?>;
            var solicitudes_sugerencia_fecha = <?php echo json_encode($solicitudes_sugerencia_fecha); ?>;

            var solicitudes_en_proceso_fecha = <?php echo json_encode($solicitudes_en_proceso_fecha); ?>;
            var solicitudes_terminada_fecha = <?php echo json_encode($solicitudes_terminada_fecha); ?>;

            var options = {
                series: [{
                        name: "Felicitaciones",
                        data: [solicitudes_felicitacion_fecha],
                    },
                    {
                        name: "Reclamos",
                        data: [solicitudes_reclamos_fecha],
                    },
                    {
                        name: "Sugerencias",
                        data: [solicitudes_sugerencia_fecha],
                    },
                    {
                        name: "Terminada",
                        data: [solicitudes_terminada_fecha],
                    },
                    {
                        name: "En proceso",
                        data: [solicitudes_en_proceso_fecha],
                    },
                    {
                        name: "Total de solicitudes",
                        data: [cantidad_solicitudes_fecha],
                    },
                ],
                chart: {
                    type: "bar",
